...
### Recommended wording for the figure

Use the updated category names in the legend:

- **Daytime low-radiation anomaly**: long daytime zero/near-zero runs, sudden radiation drops, and likely weather-related low solar.
- **Night-time radiation anomaly**: night records with solar radiation above the configured QC threshold; these are set to `NA` in `SlrW_Avg_clean`.
- **Missing data/timestamp-gap anomaly**: missing daytime solar values and estimated missing timestamps.
- **Spike/out-of-range radiation anomaly**: sudden positive jumps or daytime values outside the configured physical range.
